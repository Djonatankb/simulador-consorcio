...
/**
 * Vincula um Contato a um Lead no Kommo CRM (API v4 Link Endpoint)
 */
async function vincularContatoLeadKommo(leadId, contactId) {
  ...
  return await conectarKommo(`leads/${leadId}/link`, 'post', payload);
}

/**
 * Adiciona uma nota com os dados de UTM no Lead do Kommo CRM
 */
async function adicionarNotaUtmKommo(leadId, utms) {
  if (!leadId || !utms || typeof utms !== 'object') return null;

  const textoUtm = Object.entries(utms)
    .filter(([_, v]) => String(v || '').trim())
    .map(([k, v]) => `${k}: ${String(v).trim()}`)
    .join('\n');

  if (!textoUtm) return null;
  ...
  const payload = [
    {
      ...
      note_type: 'common',
      params: {
        text: `📌 Origem UTM / Rastreamento:\n${textoUtm}`
      }
    }
  ];

  return await conectarKommo(`leads/${leadId}/notes`, 'post', payload);
}
 ...
 * - Se não tiver Lead vinculado, cria o Lead na fase 109917515 e vincula ao contato existente via /leads/{id}/link.
 * Se o contato não existir: cria Lead na fase 109917515 + Contato e vincula via /leads/{id}/link.
 */
async function buscarOuCriarLeadKommo(nome, email, telefone, tipo, utms = null) {
  ...
  try {
    ...
    if (contatoExistente && contatoExistente.id) {
      ...
      if (leadExistenteId) {
        console.log(`Lead existente localizado (ID: ${leadExistenteId}). Consultando fase atual...`);
        
        // Consulta os detalhes do lead existente para obter o status_id (fase atual)
        const leadExistente = await conectarKommo(`leads/${leadExistenteId}`, 'get');
        const statusAtual = Number(leadExistente?.status_id || 0);

        const fasesPermitidas = [109093603, 109093599];
        const podeMudarFase = fasesPermitidas.includes(statusAtual);

        const tagsParaLead = podeMudarFase
          ? montarTagsUtm() // Inclui 'Simulador Consórcio' e 'Mensagem Simulador'
          : [{ name: 'Simulador Consórcio' }]; // Omitida a tag 'Mensagem Simulador' para leads em fases avançadas

        // Objeto de atualização do Lead
        const patchData = {
          id: Number(leadExistenteId),
          name: `Consórcio ${tipo || 'Imóvel'} - ${nome || 'Lead'}`,
          pipeline_id: pipelineId,
          _embedded: { tags: tagsParaLead }
        };

        if (podeMudarFase) {
          console.log(`Lead ${leadExistenteId} está na fase permitida (${statusAtual}). Movendo para a fase ${stageInicial}...`);
          patchData.status_id = stageInicial;
        } else {
          console.log(`Lead ${leadExistenteId} está na fase ${statusAtual} (não está nas fases 109093603 ou 109093599). Mantendo fase original e omitindo a tag 'Mensagem Simulador'.`);
        }
        ...
        // Contato existe mas não tem Lead vinculado -> cria Lead na fase 109917515, vincula via /leads/{id}/link
        const postData = {
          name: `Consórcio ${tipo || 'Imóvel'} - ${nome || 'Lead'}`,
          pipeline_id: pipelineId,
        ...
        if (novoLeadId) {
          console.log(`Lead ${novoLeadId} criado para contato existente ${contatoExistente.id}. Vinculando via /leads/${novoLeadId}/link...`);
          await vincularContatoLeadKommo(novoLeadId, contatoExistente.id);
        }
        return novoLeadId;
      }
    }
...
/**
 * Cria Lead na fase inicial (109917515) no Kommo e vincula um novo Contato (Nome, E-mail, Telefone) via /leads/{id}/link
 */
async function criarLeadKommo(nome, email, telefone, tipo, stageId = KOMMO_STAGE_INICIAL_DEFAULT, utms = null) {
  ...
  try {
    ...
    const respContato = await conectarKommo('contacts', 'post', payloadContato);
    const contactId = respContato?._embedded?.contacts?.[0]?.id || respContato?.[0]?.id;

    if (contactId) {
      console.log(`Vincular Contato ${contactId} ao Lead ${leadId} via Kommo Link API...`);
      await vincularContatoLeadKommo(leadId, contactId);
    } else {
      console.warn(`Contato criado para o Lead ${leadId}, mas ID do contato não foi obtido.`);
    }
  } catch (errContato) {
...
module.exports = {
  ...
  atualizarLeadKommo,
  vincularContatoLeadKommo,
  adicionarNotaUtmKommo
};
