...
async function buscarOuCriarLeadKommo(nome, email, telefone, tipo) {
  ...
  const stageInicial = Number(getEnv('KOMMO_STAGE_INICIAL', KOMMO_STAGE_INICIAL_DEFAULT));
  ...
  try {
    ...
    if (contatoExistente && contatoExistente.id) {
      ...
      if (leadExistenteId) {
        ...
        // Move o Lead existente para a fase 109093611
        ...
        // Contato existe mas não tem Lead vinculado -> cria Lead na fase 109093611 e vincula
...
async function criarLeadKommo(nome, email, telefone, tipo, stageId = KOMMO_STAGE_INICIAL_DEFAULT) {
  ...
  // 1. Criar o Lead na fase inicial (109093611)
  const payloadLead = [
    {
      name: `Consórcio ${tipo || 'Imóvel'} - ${nome || 'Lead'}`,
      pipeline_id: pipelineId,
...
/**
 * Atualiza o Lead no Kommo CRM: move para Transmissão (109093615), define valor e ajusta nome
 */
